...
backup_mqlaunch() {
  ...
  backup_file="$BASE_DIR/terminal/launchers/mqlaunch-$stamp.sh.bak"
...
if 'backup_mqlaunch() {' not in text:
    marker = '# --- Menus --------------------------------------------------'
    if marker not in text:
        print("Could not find menu marker.")
        sys.exit(1)
    text = text.replace(marker, backup_func + marker, 1)

# ------------------------------------------------------------
# 2) Replace print_dev_menu()
# ------------------------------------------------------------
...
echo
echo "Verify with:"
echo 'grep -n "backup_mqlaunch\|Backup mqlaunch\|backup-mqlaunch\|Select dev option \[0-10\]" "$HOME/macos-scripts/terminal/launchers/mqlaunch.sh"'
