...
mq_dirty_severity_color() {
  local severity="$1"
  case "$severity" in
    STABLE) printf '%s' "$NEON_GREEN" ;;
    LOW) printf '%s' "$NEON_CYAN" ;;
    MEDIUM) printf '%s' "$NEON_YELLOW" ;;
    HIGH|CRITICAL) printf '%s' "$NEON_RED" ;;
    *) printf '%s' "$NEON_WHITE" ;;
  esac
}
...
# Handles mqlaunch dashboard v6.
mqlaunch_dashboard_v6() {
  ...
  echo -e "${NEON_YELLOW}${C_BOLD}${mem_widget}${C_RESET}   ${NEON_CYAN}${C_BOLD}${batt_widget}${C_RESET}   ${C_DIM}adaptive layout active${C_RESET}"
  echo
  ...
  echo -e "${NEON_RED}${C_BOLD}>>> READY${C_RESET} ${C_DIM}// neon command surface stable // severity-aware widgets live${C_RESET}"
  echo -e "${NEON_CYAN}${C_BOLD}$(mq_repeat "═" "$width")${C_RESET}"
}
